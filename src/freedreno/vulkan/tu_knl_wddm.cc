...
tu_wddm_context_get_gpu_timestamp(struct tu_wddm_context *context, uint64_t *ticks)
{
   ...
   if (ticks)
      *ticks = 0;
   /* Runtime-v1 has no clock callback. Never send its opaque context to KMT. */
   if (ticks && tu_wddm_shared_context(context))
      return UINT32_C(0xc00000bb); /* STATUS_NOT_SUPPORTED */
                           ...
                           uint64_t size, struct tu_wddm_allocation *allocation)
{
   ...
   allocation->last_create_status = UINT32_C(0x80070057); /* E_INVALIDARG */
   if (!tu_wddm_context_get_info(context)) return false;
   mwd_allocation reply = {};
   auto *device = context->device;
   int32_t hr = device->callbacks.retain(device->runtime_owner, token, &reply);
   ...
   allocation->last_create_status = UINT32_C(0x80070057);
   hr = device->callbacks.release(device->runtime_owner, token);
   allocation->last_destroy_status = static_cast<uint32_t>(hr);
   if (hr < 0) {
      allocation->context = context;
      allocation->runtime_token = token;
   }
   return false;
}
                          ...
                          struct tu_wddm_allocation *allocation)
{
   ...
   if (tu_wddm_shared_context(context)) {
      allocation->last_create_status = UINT32_C(0x80070057); /* E_INVALIDARG */
      if (!desc || !desc->size || desc->size > UINT32_MAX - UINT64_C(4095) ||
          desc->alignment < 4096 || desc->alignment > 65536 ||
          (desc->alignment & (desc->alignment - 1)) ||
          (desc->requested_iova & (desc->alignment - 1)) || desc->format || desc->width ||
          desc->height || desc->pitch || desc->refresh_rate_numerator || desc->refresh_rate_denominator ||
          (desc->flags & ~14u) || !(desc->flags & 4u) ||
          !tu_wddm_context_get_info(context)) return false;
      mwd_allocation reply = {};
      auto *device = context->device;
      ...
      if ((!desc->requested_iova || reply.address == desc->requested_iova) &&
          !(reply.address & (desc->alignment - 1)) && reply.flags == desc->flags &&
          tu_wddm_shared_allocation(context, &reply, desc->size, allocation)) {
         allocation->private_info.Alignment = desc->alignment;
         return true;
      }
      allocation->last_create_status = UINT32_C(0x80070057); /* E_INVALIDARG */
      if (reply.token) {
         hr = device->callbacks.release(device->runtime_owner, reply.token);
         allocation->last_destroy_status = static_cast<uint32_t>(hr);
         if (hr < 0) {
            allocation->context = context;
            allocation->runtime_token = reply.token;
         }
      }
      return false;
   }
...
tu_wddm_allocation_lock(struct tu_wddm_allocation *allocation, void **map)
{
   if (map != NULL)
      *map = NULL;
   if (allocation && allocation->runtime_token && tu_wddm_shared_context(allocation->context)) {
      if (!map || allocation->locked ||
          !(allocation->private_info.Flags & VIOGPU_WDDM_ALLOCATION_CPU_VISIBLE)) return false;
      auto *device = allocation->context->device;
      uint32_t handle = 0;
      ...
      if (*map && handle) return true;
      /* Successful callback acquired a lock even if its reply was malformed.
       * Keep that lock owner if rollback fails so teardown can retry it. */
      if (device->callbacks.unmap(device->runtime_owner, allocation->runtime_token) >= 0)
...
tu_wddm_device_get_gpu_timestamp(struct tu_device *dev, uint64_t *ts)
{
   ...
   if (!dev || !ts || !dev->wddm_initialized)
      return -EINVAL;
   if (dev->wddm_runtime_owner || !dev->physical_device->wddm_has_gpu_timestamp)
      return -ENOSYS;
...
{
   *out_bo = NULL;
   const uint64_t alignment = (flags & TU_BO_ALLOC_BDA_64K) ? 65536 : 4096;
   if (!size || size > UINT32_MAX - UINT64_C(4095))
      return vk_error(dev, VK_ERROR_OUT_OF_DEVICE_MEMORY);
   if (client_iova & (alignment - 1))
   ...
   *bo = {};
   ...
   desc.size = size; desc.alignment = alignment; desc.requested_iova = client_iova;
   ...
   mtx_lock(&dev->bo_mutex);
   if (ok || allocation->runtime_token) {
   ...
   mtx_unlock(&dev->bo_mutex);
   if (!ok) {
      const uint32_t status = allocation->last_create_status;
      if (allocation->runtime_token)
         return vk_device_set_lost(&dev->vk,
                                   "runtime allocation rollback failed: HRESULT=0x%08x",
                                   allocation->last_destroy_status);
      vk_free(&dev->vk.alloc, allocation);
      if (vk_device_is_lost(&dev->vk) || tu_wddm_device_check_status(dev) != VK_SUCCESS ||
          vk_device_is_lost(&dev->vk))
         return VK_ERROR_DEVICE_LOST;
      if (import_token)
         return vk_error(dev, VK_ERROR_INVALID_EXTERNAL_HANDLE);
      if (status == UINT32_C(0x8007000e)) /* E_OUTOFMEMORY */
         return vk_error(dev, VK_ERROR_OUT_OF_DEVICE_MEMORY);
      return vk_errorf(dev, VK_ERROR_UNKNOWN,
                       "runtime allocation failed: HRESULT=0x%08x", status);
   }
